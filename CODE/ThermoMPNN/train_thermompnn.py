...
def train(cfg):
    ...
    # Extract embeddings for a PDB structure
    dataset_splits = ["train", "val", "test"]

    for split in dataset_splits:
        if "fireprot" in cfg.datasets:
            # FireProtDataset
            pdb_dataset = FireProtDataset(cfg, split)
        elif "megascale" in cfg.datasets:
            # MegaScaleDataset
            pdb_dataset = MegaScaleDataset(cfg, split)
        else:
            raise ValueError("Invalid dataset specified in the config file.")

        pdb_loader = DataLoader(pdb_dataset, collate_fn=collate_fn, num_workers=0)

        with torch.no_grad():
            for batch in pdb_loader:
                pdb, _ = batch[0]
                embeddings = model_pl.model.get_embeddings(pdb)
                
                # Save embeddings to a file in the specified embeddings directory
                embeddings_dir = cfg.platform.embeddings_dir
                os.makedirs(embeddings_dir, exist_ok=True)
                embeddings_file = os.path.join(embeddings_dir, f"embeddings_{split}.pt")
    ...
    if 'two_stage' in cfg.training:  # sequential combo training
        if cfg.training.two_stage:
            ...
            # Extract embeddings after two-stage training
            dataset_splits = ["train", "val", "test"]
            ...
            for split in dataset_splits:
                ...
                with torch.no_grad():
                    for batch in pdb_loader:
                        ...
                        embeddings_dir = cfg.platform.embeddings_dir
                        os.makedirs(embeddings_dir, exist_ok=True)
                        embeddings_file = os.path.join(embeddings_dir, f"embeddings_two_stage_{split}.pt")
                        torch.save(embeddings, embeddings_file)
                        # print(f"Embeddings saved to: {embeddings_file}")
